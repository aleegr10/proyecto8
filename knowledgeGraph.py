...
def read_json_triplets(file_path):
    try:
        with open(file_path, "r") as file:
            data = json.load(file)
            return [
                (item["sujeto"], item["predicado"], item["objeto"])
                for item in data
                if "sujeto" in item and "predicado" in item and "objeto" in item
            ]
...
files = [f for f in os.listdir("./triplets") if f.startswith("triplets") and f.endswith(".txt")]

kg = []
for file_name in files:
    file_path = os.path.join(os.getcwd(), "./triplets/"+file_name)
...
## Preguntar
question = "¿Cuál es la última noticia sobre el Valencia Basket?"
