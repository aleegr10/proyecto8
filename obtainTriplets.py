...
def main():
    ...
    archivos = [
        ...
        if os.path.isfile(os.path.join("./responses", f)) and f.startswith("response") and f.endswith(".md")
    ]

    # Por cada archivo, leer el contenido y almacenarlo en la variable instrucciones
    instrucciones = ""
    for archivo in archivos:
        with open("./responses/"+archivo, "r", encoding="utf-8") as file:
            instrucciones = file.read()
        print(f"Procesando {archivo}")
        triplets = llamada_modelo(
            obtener_prompt("systemPrompt.txt"), instrucciones, client
        )
